...
function walk(dir, exts = ['.html', '.vue', '.ts', '.js', '.tsx', '.jsx', '.json']) {
  const results = [];
  const list = fs.readdirSync(dir, { withFileTypes: true });
  for (const ent of list) {
    if (ent.name === 'node_modules' || ent.name === '.output' || ent.name === '.nuxt' || ent.name === '.git') continue;
...
function extractUrls(text) {
  const urls = new Set();
  const regex = /(https:\/\/www\.ruangoffice\.com[^"'\s)]+)|(https:\/\/ruangoffice\.com[^"'\s)]+)/g;
  let m;
  while ((m = regex.exec(text)) !== null) {
    const url = (m[1] || m[2]).trim();
    if (url.startsWith('https://www.ruangoffice.com') || url.startsWith('https://ruangoffice.com')) {
      urls.add(url);
    }
  }
...
function rewriteFile(filePath, replacements) {
  ...
  for (const [remote, localRel] of replacements) {
    ...
    const pattern = new RegExp(remote.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
...
(async function main() {
  ...
  console.log(`Found ${urls.length} ruangoffice.com URLs. Downloading to ${path.relative(PROJECT_ROOT, TARGET_BASE)} ...`);
  const results = [];
  ...
  function shouldDownload(u) {
    ...
    try {
      ...
      const allow = new Set(['.css', '.js', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico', '.webp', '.woff', '.woff2', '.ttf', '.otf', '.json', '.map']);
  ...
  for (const url of urls) {
    ...
    try {
      ...
      results.push({ url, dest: path.relative(PROJECT_ROOT, dest), status: r.skipped ? 'skipped' : 'downloaded' });
      console.log(`${r.skipped ? 'SKIP' : 'OK  '} ${url} -> ${path.relative(PROJECT_ROOT, dest)}`);
    } catch (e) {
      results.push({ url, dest: path.relative(PROJECT_ROOT, dest), status: 'failed', error: e.message });
  ...
  if (REWRITE) {
    ...
    const replacePairs = results
      ...
      .map(r => {
        const rel = '/' + path.relative(PUBLIC_DIR, path.join(PROJECT_ROOT, r.dest)).replace(/\\/g, '/');
        return [r.url, rel];
      });
  ...
  const reportPath = path.join(PROJECT_ROOT, 'public', 'assets', 'ruangoffice', 'download-report.json');
